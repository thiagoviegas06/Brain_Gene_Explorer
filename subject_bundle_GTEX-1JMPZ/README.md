# `subject_bundle_GTEX-1JMPZ/` — local viz assets for one GTEx subject

A self-contained download bundle (~44 MB) for experimenting with the 3D gene-expression viewer locally. Default subject: **GTEX-1JMPZ**. Includes atlas geometry, one subject across all three model variants, gene-list presets, and the canonical parcel-order mapping.
...
```
subject_bundle_GTEX-1JMPZ/
├── atlas_info/
│   ├── atlas-4S156Parcels_dseg_reformatted.csv
│   └── atlas-4S156Parcels_space-MNI152NLin6Asym_dseg.nii.gz
├── cache/
│   ├── naive/    GTEX-1JMPZ.npz + .json
│   ├── dlam/     GTEX-1JMPZ.npz + .json
│   └── plam/     GTEX-1JMPZ.npz + .json
├── gene_lists/
...
## `cache/{naive,dlam,plam}/GTEX-1JMPZ.npz`
...
```python
import numpy as np
d = np.load("cache/plam/GTEX-1JMPZ.npz", allow_pickle=True)
```
