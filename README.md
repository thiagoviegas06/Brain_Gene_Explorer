...
```
Brain_Gene_Explorer/          # repo root — run everything from here
...
Subject data lives inside the repo root (gitignored):
...
`app.py` expects the subject data folders to live inside the repo root, alongside `app.py`:
...
If those folders are missing, the app will not be able to load the atlas alignment,
