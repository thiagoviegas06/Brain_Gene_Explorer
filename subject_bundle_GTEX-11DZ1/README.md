# `subject_bundle_GTEX-11DZ1/` — local viz assets for one GTEx subject

A self-contained download bundle (~44 MB) for experimenting with the 3D gene-expression viewer locally. Default subject: **GTEX-11DZ1**. Includes atlas geometry, one subject across all three model variants, gene-list presets, and the canonical parcel-order mapping.
...
```
subject_bundle_GTEX-11DZ1/
├── atlas_info/
│   ├── atlas-4S156Parcels_dseg_reformatted.csv
│   └── atlas-4S156Parcels_space-MNI152NLin6Asym_dseg.nii.gz
├── cache/
│   ├── naive/    GTEX-11DZ1.npz + .json
│   ├── dlam/     GTEX-11DZ1.npz + .json
│   └── plam/     GTEX-11DZ1.npz + .json
├── gene_lists/
...
## `cache/{naive,dlam,plam}/GTEX-11DZ1.npz`
...
```python
import numpy as np
d = np.load("cache/plam/GTEX-11DZ1.npz", allow_pickle=True)
```
