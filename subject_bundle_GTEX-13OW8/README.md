# `subject_bundle_GTEX-13OW8/` — local viz assets for one GTEx subject

A self-contained download bundle (~45 MB) for experimenting with the 3D gene-expression viewer locally. Subject: **GTEX-13OW8**. Includes atlas geometry, one subject across all three model variants, gene-list presets, and the canonical parcel-order mapping.
...
├── cache/
│   ├── naive/    GTEX-13OW8.npz + .json
│   ├── dlam/     GTEX-13OW8.npz + .json
│   └── plam/     GTEX-13OW8.npz + .json
├── gene_lists/
...
```python
import numpy as np
d = np.load("cache/plam/GTEX-13OW8.npz", allow_pickle=True)
```
