# `subject_bundle/` — local viz assets for one GTEx subject

A self-contained download bundle (~44 MB) for experimenting with the 3D gene-expression viewer locally. Default subject: **GTEX-1117F**. Includes atlas geometry, one subject across all three model variants, gene-list presets, and the canonical parcel-order mapping.
...
├── cache/
│   ├── naive/    GTEX-1117F.npz + .json
│   ├── dlam/     GTEX-1117F.npz + .json
│   └── plam/     GTEX-1117F.npz + .json
├── gene_lists/
...
```python
import numpy as np
d = np.load("cache/plam/GTEX-1117F.npz", allow_pickle=True)
```
